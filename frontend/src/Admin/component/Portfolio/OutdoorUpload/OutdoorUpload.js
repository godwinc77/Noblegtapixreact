...
import "./outdoorupload.css";

const OutdoorUpload = () => {
  const baseURL = "http://localhost:4000/outdoorimages/";
  ...
  const handleDeleteImage = async (imageId) => {
    try {
      ...
      await axios.delete(
        `http://localhost:4000/outdoor-delete-image/${imageId}`
      );
    ...
    try {
      ...
      for (const selectedImage of images) {
        ...
        const response = await axios.post(
          "http://localhost:4000/outdoor-upload-image",
          formData,
          {
  ...
  const getImage = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:4000/outdoor-get-image"
      );

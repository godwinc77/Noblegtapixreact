...
import "./makeupupload.css";

const MakeupUpload = () => {
  const baseURL = "http://localhost:4000/makeupimages/";
  ...
  const handleDeleteImage = async (imageId) => {
    try {
      ...
      await axios.delete(
        `http://localhost:4000/makeup-delete-image/${imageId}`
      );
    ...
    try {
      ...
      for (const selectedImage of images) {
        ...
        const response = await axios.post(
          "http://localhost:4000/makeup-upload-image",
          formData,
          {
  ...
  const getImage = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:4000/makeup-get-image"
      );

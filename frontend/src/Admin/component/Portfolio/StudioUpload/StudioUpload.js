...
const StudioUpload = () => {
  const baseURL = "http://localhost:4000/studioimages/";
  ...
  const handleDeleteImage = async (imageId) => {
    try {
      ...
      await axios.delete(
        `http://localhost:4000/studio-delete-image/${imageId}`
      );
    ...
    try {
      ...
      for (const selectedImage of images) {
        ...
        const response = await axios.post(
          "http://localhost:4000/studio-upload-image",
          formData,
          {
  ...
  const getImage = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:4000/studio-get-image"
      );

...
const HomeBackground = () => {
  ...
  const handleDeleteImage = async (imageId) => {
    try {
      ...
      await axios.delete(`http://localhost:4000/delete-image/${imageId}`);
  ...
  const getImage = async () => {
    try {
      ...
      if (
        ...
        setAllImage(response.data.data.slice(-5)); // Keep only the last 5 images
      } else {
  ...
  return (
    <div className="background__con">
      <div className="background__con2">
        <form className="background__upload" onSubmit={submitImage}>
          <div className="background__upload__input">
            <input
              type="file"
              accept="image/*"
              multiple
        ...
        <div className="background__display">
          {loading ? (
            <p>Loading...</p>
            ...
            allImage.map((imageId, index) => {
              ...
              return (
                <div className="background__display_img" key={index}>
                  <img
                    src={imageUrl}

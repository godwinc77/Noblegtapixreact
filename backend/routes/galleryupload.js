...
router.post("/gallerys-upload-image", upload.single("image"), (req, res) => {
  ...
  fs.readFile(imagePath, (err, data) => {
    ...
    const sql = "INSERT INTO gallery (images) VALUES (?)";
...
router.get("/gallerys-get-image", async (req, res) => {
  try {
    const query = "SELECT id FROM gallery";
...
router.get("/gallerysimages/:id", (req, res) => {
  const imageId = req.params.id;
  const query = "SELECT images FROM gallery WHERE id = ?";
  pool.query(query, [imageId], (err, result) => {
    ...
    const imageBuffer = result[0].images; // Correct field name
...
router.delete("/gallerys-delete-image/:imageId", (req, res) => {
